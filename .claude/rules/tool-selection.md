...
| File read/write | Editor tooling (`Read`, `Write`, `Edit`) | — | Never terminal for writes |
| Code search | Search tools (`Grep`, `Glob`) | Terminal `grep` | Prefer structured tools for accuracy |
| Build / test / lint | Terminal (`Bash`) | — | Inherently terminal-native; use `commands.*` from config |
| Git operations | Terminal (`Bash`) | — | `git` CLI is the canonical interface |
| Package management | Terminal (`Bash`) | — | `npm`, `dotnet`, `pip` are terminal-native |
