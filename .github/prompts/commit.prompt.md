...
Uses **Implementer**. Stage file changes and create a well-formed Conventional Commit with appropriate type, scope, and description.
...
2. Run `git diff` (or `git diff --cached` if already staged) to review changes
...
Stage and compose commit message.
...
Use the workspace's defined scopes (from workspace.config.yaml or git conventions). Common scopes:
