...
# render-instructions.sh — Replace consumer-fill tokens in instruction files
#
# Usage:
...
# then finds instruction files at workspace root that contain consumer-fill
# {{{placeholder}}} tokens and replaces them with real values in-place.
#
# Designed to run once after creating a workspace from a template. Re-running
# is safe (idempotent) — files without tokens are skipped.
#
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      ...
      echo ""
      echo "Replace consumer-fill tokens in instruction files with values"
      echo "from workspace.config.yaml. Run once after creating a workspace"
      echo "from a template."
      echo ""
      echo "Options:"
...
while IFS= read -r file; do
  [[ -z "$file" ]] && continue
  RENDER_FILES+=("$file")
done < <(grep -rl -E "$CONSUMER_TOKEN_PATTERN" "$WORKSPACE_DIR" \
  --include='*.md' \
  --exclude-dir='.git' \
  --exclude-dir='repos' \
  --exclude-dir='.tmp' \
...
if [[ ${#RENDER_FILES[@]} -eq 0 ]]; then
  log_success "No files with consumer-fill tokens found — nothing to render"
  exit 0
fi

log_info "Found ${#RENDER_FILES[@]} file(s) with consumer-fill tokens"

# ─── Portable sed in-place ─────────────────────────────────────────
# macOS sed requires -i '', GNU sed requires -i (no arg)
...
# ─── Render each file in-place ────────────────────────────────────
RENDERED_COUNT=0
SKIPPED_COUNT=0
UNFILLED_REPORTS=()

for src_file in "${RENDER_FILES[@]}"; do
  rel_path="${src_file#"$WORKSPACE_DIR"/}"

  # Work on a temp copy so we can diff before writing
  tmp_file=$(mktemp)
  cp "$src_file" "$tmp_file"

  # Simple token replacements (single-line values)
  [[ -n "$PROJECT_NAME" ]] && \
    sed_inplace "s|{{{project_name}}}|$(escape_sed "$PROJECT_NAME")|g" "$tmp_file"

  [[ -n "$PROJECT_PURPOSE" ]] && \
    sed_inplace "s|{{{project_purpose}}}|$(escape_sed "$PROJECT_PURPOSE")|g" "$tmp_file"

  [[ -n "$BUILD_COMMAND" ]] && \
    sed_inplace "s|{{{build_command}}}|$(escape_sed "$BUILD_COMMAND")|g" "$tmp_file"

  [[ -n "$TEST_COMMAND" ]] && \
    sed_inplace "s|{{{test_command}}}|$(escape_sed "$TEST_COMMAND")|g" "$tmp_file"

  [[ -n "$RUN_COMMAND" ]] && \
    sed_inplace "s|{{{run_command}}}|$(escape_sed "$RUN_COMMAND")|g" "$tmp_file"

  [[ -n "$LINT_COMMAND" ]] && \
    sed_inplace "s|{{{lint_command}}}|$(escape_sed "$LINT_COMMAND")|g" "$tmp_file"

  [[ -n "$PROJECT_ROOT" ]] && \
    sed_inplace "s|{{{project_root}}}|$(escape_sed "$PROJECT_ROOT")|g" "$tmp_file"

  [[ -n "$BASE_BRANCH" ]] && \
    sed_inplace "s|{{{base_branch}}}|$(escape_sed "$BASE_BRANCH")|g" "$tmp_file"

  [[ -n "$BRANCH_PATTERN" ]] && \
    sed_inplace "s|{{{branch_pattern}}}|$(escape_sed "$BRANCH_PATTERN")|g" "$tmp_file"
  ...
  # Add generated header if not already present
  HEADER_COMMENT="<!-- GENERATED by tools/render-instructions.sh — edit workspace.config.yaml to change consumer values -->"
  if ! grep -qF "GENERATED by tools/render-instructions.sh" "$tmp_file" 2>/dev/null; then
  ...
  # Check for unfilled consumer tokens
  ...
  if [[ "$DRY_RUN" == true ]]; then
    diff_output=$(diff "$src_file" "$tmp_file" 2>/dev/null || true)
    ...
    if ! diff -q "$src_file" "$tmp_file" >/dev/null 2>&1; then
...
echo "  Rendered:  $RENDERED_COUNT file(s)"
[[ $SKIPPED_COUNT -gt 0 ]] && echo "  Unchanged: $SKIPPED_COUNT file(s)"
echo ""

# Report unfilled consumer tokens
REMAINING=0
...
elif [[ "$DRY_RUN" != true && $RENDERED_COUNT -gt 0 ]]; then
